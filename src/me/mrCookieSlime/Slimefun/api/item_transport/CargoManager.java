...
public class CargoManager {
    ...
    public static ItemSlot withdraw(Block node, BlockStorage storage, Block target, int index) {
        if (storage.hasUniversalInventory(target)) {
            ...
            String id = BlockStorage.checkID(node);
                for (int slot = 0; slot < inv.getContents().length; slot++) {
                    if (inv.getType().equals(InventoryType.FURNACE) && id.equals("CARGO_NODE_INPUT_ADVANCED")&& !hasrightslot(node, slot)) continue;
    ...
    public static boolean matchesFilter(Block block, ItemStack item, int index) {
        ...
        if (id.equals("CARGO_NODE_OUTPUT") || id.equals("CARGO_NODE_INPUT")) return true;
        ...
        if (BlockStorage.getBlockInfo(block, "filter-type").equals("whitelist")) {
            ...
            if (index >= 0) {
                ...
                for (ItemStack stack : items) {
                    if (SlimefunManager.isItemSimiliar(item, stack, lore, data ? DataType.ALWAYS : DataType.NEVER))
                        return true;
                }
                return false;
            }
