            ...
            switch (e.getEntityType()) {
                case ZOMBIE -> {
                    if (random.nextInt(100) < chanceZombie.getValue()) {
                        e.getDrops().add(ItemStack.of(Material.ZOMBIE_HEAD));
                    }
                }
                case SKELETON -> {
                    if (random.nextInt(100) < chanceSkeleton.getValue()) {
                        e.getDrops().add(ItemStack.of(Material.SKELETON_SKULL));
                    }
                }
                case CREEPER -> {
                    if (random.nextInt(100) < chanceCreeper.getValue()) {
                        e.getDrops().add(ItemStack.of(Material.CREEPER_HEAD));
                    }
                }
                case WITHER_SKELETON -> {
                    if (random.nextInt(100) < chanceWitherSkeleton.getValue()) {
                        e.getDrops().add(ItemStack.of(Material.WITHER_SKELETON_SKULL));
                    }
                }
                case PIGLIN -> {
                    if (Slimefun.getMinecraftVersion().isAtLeast(MinecraftVersion.MINECRAFT_1_20) &&
                        random.nextInt(100) < chancePiglin.getValue()) {
                        e.getDrops().add(ItemStack.of(Material.PIGLIN_HEAD));
                    }
                }
                case PLAYER -> {
