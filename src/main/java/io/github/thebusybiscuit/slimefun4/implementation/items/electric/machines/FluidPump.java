...
public class FluidPump extends SimpleSlimefunItem<BlockTicker> implements InventoryBlock, EnergyNetComponent {
    ...
    private final ItemStack emptyBucket = ItemStackWrapper.wrap(StackResolver.of(Material.BUCKET));
    private final ItemStack emptyBottle = ItemStackWrapper.wrap(StackResolver.of(Material.GLASS_BOTTLE));
    ...
    private @Nonnull ItemStack getFilledBottle(@Nonnull Block fluid) {
        switch (fluid.getType()) {
            case WATER:
            case BUBBLE_COLUMN:
                ItemStack waterBottle = StackResolver.of(Material.POTION);
                ...
                return waterBottle;
            default:
                return StackResolver.of(Material.GLASS_BOTTLE);
        }
    }

    private @Nonnull ItemStack getFilledBucket(@Nonnull Block fluid) {
        return switch (fluid.getType()) {
            case LAVA -> StackResolver.of(Material.LAVA_BUCKET);
            case WATER,
                BUBBLE_COLUMN -> StackResolver.of(Material.WATER_BUCKET);
            default ->
                // Fallback for any new liquids
                StackResolver.of(Material.BUCKET);
        };
    }
