...
public final class ChestMenuUtils {
    ...
    public static @Nonnull ItemStack getBackButton(@Nonnull Player p, String... lore) {
        return new CustomItem(BACK_BUTTON, "&7\u21E6 " + Slimefun.getLocalization().getMessage(p, "guide.back.title"), lore);
    }

    public static @Nonnull ItemStack getMenuButton(@Nonnull Player p) {
        return new CustomItem(MENU_BUTTON, ChatColor.YELLOW + Slimefun.getLocalization().getMessage(p, "guide.title.settings"), "", "&7\u21E8 " + Slimefun.getLocalization().getMessage(p, "guide.tooltips.open-category"));
    }
