...
class Hologram {
    ...
    ArmorStand getArmorStand() {
        Entity n = (Entity) ReflectionCompat.invokeStatic(Bukkit.class, "getEntity", uniqueId);

        if (n instanceof ArmorStand && n.isValid()) {
            ArmorStand armorStand = (ArmorStand) n;            this.lastAccess = System.currentTimeMillis();
            return armorStand;
        } else {
            this.lastAccess = 0;
            return null;
