...
public class BlockPlacer extends SlimefunItem {
    ...
    private boolean isAllowed(@Nonnull Block facedBlock, @Nonnull Material type) {
        if (!type.isBlock()) {
             ...
             */
            return !Boolean.TRUE.equals(ReflectionCompat.invoke(facedBlock.getRelative(BlockFace.DOWN), "isPassable"));
                ...
                if (item.hasItemMeta()) {
                    ...
                    if (meta.hasDisplayName()) {
                        ...
                        if (blockState.getState() instanceof Nameable) {
                            Nameable nameable = (Nameable) blockState.getState();                            nameable.setCustomName(meta.getDisplayName());
