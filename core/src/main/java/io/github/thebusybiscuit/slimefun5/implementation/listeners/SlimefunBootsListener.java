...
public class SlimefunBootsListener implements Listener {
    ...
    public void onEnderPearlDamage(EntityDamageByEntityEvent e) {
        if (e.getDamager() instanceof EnderPearl && e.getEntity() instanceof Player) {
            Player p = (Player) e.getEntity();            SlimefunItem boots = SlimefunItem.getByItem(p.getInventory().getBoots());
        ...
        if (e.getEntity() instanceof Player) {
            ...
            if (boots != null) {
                ...
                if (boots instanceof StomperBoots) {
                    StomperBoots stomperBoots = (StomperBoots) boots;                    e.setCancelled(true);
                    stomperBoots.stomp(e);
                } else if (boots instanceof LongFallBoots) {
                    LongFallBoots longFallBoots = (LongFallBoots) boots;                    e.setCancelled(true);
    ...
    public void onTrample(PlayerInteractEvent e) {
        if (e.getAction() == Action.PHYSICAL) {
            ...
            if (b != null && b.getType() == XMaterial.FARMLAND.parseMaterial()) {
