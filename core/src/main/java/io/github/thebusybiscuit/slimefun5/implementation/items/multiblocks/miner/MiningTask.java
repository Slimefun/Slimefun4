...
class MiningTask implements Runnable {
    ...
    public void run() {
        ...
        queue.thenRun(() -> {
            try {
                ...
                for (int y = height; y > WorldCompat.getMinHeight(world); y--) {
    ...
    private void nextColumn() {
        if (x < end.getX()) {
            ...
            if (p != null) {
                SoundCompat.playFor(p, p.getLocation(), "ENTITY_ARROW_HIT_PLAYER", null, 0.4F, 1F);
        ...
        if (fuelLevel > 0) {
            if (chest.getType() == Material.CHEST) {
                ...
                if (state instanceof Chest) {
                    Chest chestState = (Chest) state;                    Inventory inv = chestState.getBlockInventory();
    ...
    private void setPistonState(@Nonnull Block block, boolean extended) {
        ...
        try {
            ...
            ParticleCompat.spawn(block.getWorld(), VersionedParticle.SMOKE, particleLoc, 20, 0.7, 0.7, 0.7, 0);

            if (block.getType() == XMaterial.MOVING_PISTON.parseMaterial()) {
                // Yeah it isn't really cool when this happens
                block.getRelative(BlockFace.UP).setType(Material.AIR);
            } else if (block.getType() == XMaterial.PISTON.parseMaterial()) {
    ...
    private void setExtended(@Nonnull Block block, @Nonnull Object piston, boolean extended) {
        ...
        SoundCompat.playAt(block.getLocation(), extended ? "BLOCK_PISTON_EXTEND" : "BLOCK_PISTON_CONTRACT", null, 0.1F, 1F);
    }
