    ...
    public void registerDefaultRecipes() {
        Slimefun.getMinecraftRecipeService().subscribe(snapshot -> {
            for (FurnaceRecipe recipe : snapshot.getRecipes(FurnaceRecipe.class)) {
                RecipeChoice choice = (RecipeChoice) ReflectionCompat.invoke(recipe, "getInputChoice");

                if (choice instanceof MaterialChoice) {
                    MaterialChoice materialChoice = (MaterialChoice) choice;                    for (Material input : materialChoice.getChoices()) {
