...
public class GuideHistory {
    ...
    private <T> void open(@Nonnull SlimefunGuideImplementation guide, @Nullable GuideEntry<T> entry) {
        if (entry == null) {
            ...
            ItemGroup group = (ItemGroup) entry.getIndexedObject();            guide.openItemGroup(profile, group, entry.getPage());
        } else if (entry.getIndexedObject() instanceof SlimefunItem) {
            SlimefunItem item = (SlimefunItem) entry.getIndexedObject();            guide.displayItem(profile, item, false);
        } else if (entry.getIndexedObject() instanceof ItemStack) {
            ItemStack stack = (ItemStack) entry.getIndexedObject();            guide.displayItem(profile, stack, entry.getPage(), false);
        } else if (entry.getIndexedObject() instanceof String) {
            String query = (String) entry.getIndexedObject();            guide.openSearch(profile, query, false);
        } else {
