...
public class Juice extends SimpleSlimefunItem<ItemConsumptionHandler> {
    ...
    public Juice(ItemGroup itemGroup, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe, @Nullable ItemStack recipeOutput) {
        ...
        if (meta instanceof PotionMeta) {
            PotionMeta potionMeta = (PotionMeta) meta;            effects = potionMeta.getCustomEffects();
        } else {
    ...
    private void removeGlassBottle(Player p, ItemStack item) {
        if (SlimefunUtils.isItemSimilar(item, HandCompat.getMainHand(p.getInventory()), true)) {
            if (HandCompat.getMainHand(p.getInventory()).getAmount() == 1) {
                HandCompat.getMainHand(p.getEquipment()).setAmount(0);
            } else {
                p.getInventory().removeItem(new ItemStack(Material.GLASS_BOTTLE, 1));
            }
        } else if (SlimefunUtils.isItemSimilar(item, HandCompat.getOffHand(p.getInventory()), true)) {
            if (HandCompat.getOffHand(p.getInventory()).getAmount() == 1) {
                HandCompat.getOffHand(p.getEquipment()).setAmount(0);
            } else {
                p.getInventory().removeItem(new ItemStack(Material.GLASS_BOTTLE, 1));
            }
        }
