...
public class SlimefunAutoCrafter extends AbstractAutoCrafter {
    ...
    protected void updateRecipe(@Nonnull Block b, @Nonnull Player p) {
        ItemStack itemInHand = HandCompat.getMainHand(p.getInventory());
        ...
        if (item != null && item.getRecipeType().equals(targetRecipeType)) {
            ...
            if (item.canUse(p, true)) {
                ...
                if (recipe != null) {
                    ...
                    menu.addItem(49, CustomItemStack.create(XMaterial.CRAFTING_TABLE.parseMaterial(), ChatColor.GREEN + Slimefun.getLocalization().getMessage(p, "messages.auto-crafting.select")));
