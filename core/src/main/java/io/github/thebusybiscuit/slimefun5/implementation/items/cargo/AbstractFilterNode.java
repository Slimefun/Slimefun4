...
abstract class AbstractFilterNode extends AbstractCargoNode {
    ...
    protected void updateBlockMenu(BlockMenu menu, Block b) {
        ...
        if (!BlockStorage.hasBlockInfo(b) || filterType == null || filterType.equals("whitelist")) {
            menu.replaceExistingItem(15, CustomItemStack.create(XMaterial.WHITE_WOOL.parseMaterial(), "&7Type: &rWhitelist", "", "&e> Click to change it to Blacklist"));
            ...
            menu.replaceExistingItem(15, CustomItemStack.create(XMaterial.BLACK_WOOL.parseMaterial(), "&7Type: &8Blacklist", "", "&e> Click to change it to Whitelist"));
            menu.addMenuClickHandler(15, (p, slot, item, action) -> {
                BlockStorage.addBlockInfo(b, FILTER_TYPE, "whitelist");
                updateBlockMenu(menu, b);
                return false;
