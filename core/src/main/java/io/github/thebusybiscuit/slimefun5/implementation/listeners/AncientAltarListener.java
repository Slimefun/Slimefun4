...
public class AncientAltarListener implements Listener {
    ...
    public void onInteract(PlayerRightClickEvent e) {
        ...
        if (b.getType() != XMaterial.ENCHANTING_TABLE.parseMaterial() && b.getType() != Material.DISPENSER) {
            return;
        }
    ...
    private void usePedestal(@Nonnull Block pedestal, @Nonnull Player p) {
        ...
        if (!stack.isPresent()) {
            ...
            if (HandCompat.getMainHand(p.getInventory()).getType() != Material.AIR) {
                // Check for pedestal obstructions
                if (!MaterialCompat.isAir(pedestal.getRelative(0, 1, 0).getType())) {
    ...
    private void useAltar(@Nonnull Block altar, @Nonnull Player p) {
        ...
        ItemStack catalyst = CustomItemStack.create(HandCompat.getMainHand(p.getInventory()), 1);
