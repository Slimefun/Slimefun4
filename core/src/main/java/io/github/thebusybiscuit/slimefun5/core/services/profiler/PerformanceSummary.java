...
class PerformanceSummary {
    ...
    private void summarizeTimings(int count, String name, PerformanceInspector inspector, Map<String, Long> map, Function<Map.Entry<String, Long>, String> formatter) {
        ...
        if (inspector instanceof PlayerPerformanceInspector) {
            PlayerPerformanceInspector playerPerformanceInspector = (PlayerPerformanceInspector) inspector;            TextComponent component = summarizeAsTextComponent(count, prefix, results, formatter);
    ...
    private TextComponent summarizeAsTextComponent(int count, String prefix, List<Map.Entry<String, Long>> results, Function<Entry<String, Long>, String> formatter) {
        ...
        if (count > 0) {
            ...
            // Java-8 universal port: use the BaseComponent[] HoverEvent ctor (present on 1.8.8 and modern)
            // rather than the 1.16+ Content-based one, which avoids the missing hover.content package.
    ...
    private String getPerformanceRating() {
        ...
        builder.append(ChatColor.DARK_GRAY)
            .append(org.apache.commons.lang.StringUtils.repeat(":", Math.max(0, rest)))
            .append(" - ")
