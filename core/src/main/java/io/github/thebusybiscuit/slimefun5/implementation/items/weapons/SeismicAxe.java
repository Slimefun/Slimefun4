...
public class SeismicAxe extends SimpleSlimefunItem<ItemUseHandler> implements NotPlaceable, DamageableItem {
    ...
    public ItemUseHandler getItemHandler() {
        return e -> {
            ...
            List<Block> blocks = p.getLineOfSight((Set<Material>) null, RANGE);
            ...
            for (int i = 2; i < blocks.size(); i++) {
                ...
                if (MaterialCompat.isAir(blockAbove.getType())) {
    ...
    private void createJumpingBlock(Block ground, Block blockAbove, int index) {
        ...
        FallingBlock block = BlockDataCompat.spawnFallingBlock(ground.getWorld(), loc, BlockDataCompat.getBlockData(ground));
    ...
    private @Nonnull Block findGround(@Nonnull Block b) {
        if (b.getType() == Material.AIR) {
            int minHeight = WorldCompat.getMinHeight(b.getWorld());
