...
public abstract class NetherStarReactor extends Reactor {
    ...
    public void extraTick(@Nonnull Location l) {
        Slimefun.runSync(() -> {
            for (Entity entity : EntityCompat.getNearbyEntities(l.getWorld(), l, 5, 5, 5, n -> n instanceof LivingEntity && n.isValid())) {
                if (entity instanceof LivingEntity) {
                    LivingEntity livingEntity = (LivingEntity) entity;                    livingEntity.addPotionEffect(new PotionEffect(PotionEffectType.WITHER, 60, 1));
                }
            }
        });
    }
