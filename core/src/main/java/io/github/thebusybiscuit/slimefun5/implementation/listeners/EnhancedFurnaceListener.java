...
public class EnhancedFurnaceListener implements Listener {
    ...
    public void onFuelBurn(FurnaceBurnEvent e) {
        ...
        if (furnace instanceof EnhancedFurnace
            && !((EnhancedFurnace) furnace).isDisabledIn(e.getBlock().getWorld())
            && ((EnhancedFurnace) furnace).getFuelEfficiency() > 0
        ) {
            EnhancedFurnace enhancedFurnace = (EnhancedFurnace) furnace;
    ...
    public void onItemSmelt(FurnaceSmeltEvent e) {
        ...
        if (sfItem instanceof EnhancedFurnace && !((EnhancedFurnace) sfItem).isDisabledIn(e.getBlock().getWorld())) {
            EnhancedFurnace enhancedFurnace = (EnhancedFurnace) sfItem;            BlockState state = PaperLib.getBlockState(e.getBlock(), false).getState();

            if (state instanceof Furnace) {
                Furnace furnace = (Furnace) state;                FurnaceInventory inventory = furnace.getInventory();
