...
public class ExplosiveBow extends SlimefunBow {
    ...
    public BowShootHandler onShoot() {
        return (e, target) -> {
            ParticleCompat.spawn(target.getWorld(), VersionedParticle.EXPLOSION, target.getLocation(), 1);
            SoundEffect.EXPLOSIVE_BOW_HIT_SOUND.playAt(target.getLocation(), SoundCategory.PLAYERS);
            int radius = range.getValue();

            Collection<Entity> entities = EntityCompat.getNearbyEntities(target.getWorld(), target.getLocation(), radius, radius, radius, this::canDamage);
