...
final class CargoUtils {
    ...
    static int[] getInputSlotRange(@Nonnull Inventory inv, @Nullable ItemStack item) {
        if (inv instanceof FurnaceInventory) {
            if (item != null && MaterialCompat.isFuel(item.getType())) {
        ...
        if (menu == null) {
            if (hasInventory(target)) {
                ...
                if (state instanceof InventoryHolder) {
                    InventoryHolder inventoryHolder = (InventoryHolder) state;                    inventory = inventoryHolder.getInventory();
    ...
    private static boolean isPotion(@Nullable ItemStack item) {
        if (item != null) {
            ...
            return type == Material.POTION || type == XMaterial.SPLASH_POTION.parseMaterial() || type == XMaterial.LINGERING_POTION.parseMaterial();
        } else {
            return false;
        }
    }
